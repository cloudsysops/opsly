...
- **Sin infra nueva** salvo decisión explícita y alineación con `VISION.md` (Compose por defecto; _Nunca_ big-bang K8s/Swarm para el control plane; excepción futura _compute plane_ solo según [ADR-027](docs/adr/ADR-027-hybrid-compute-plane-k8s.md); escalar VPS antes que complejidad).
...
| Control plane  | Compose + Traefik en VPS por defecto; K8s solo como _compute plane_ opcional futuro ([ADR-027](docs/adr/ADR-027-hybrid-compute-plane-k8s.md)) |
