...
10. **Ampliar Zero-Trust** — UI u otras rutas portal — cuando se priorice (`resolveTrustedPortalSession` + `tenantSlugMatchesSession` si hay `[slug]`).
...
2. **API** — `GET /api/portal/me`, `POST /api/portal/mode`, `GET /api/portal/usage`, invitaciones `POST /api/invitations` + Resend; **`lib/portal-me.ts`**, **`portal-auth.ts`**, **`cors-origins.ts`**, **`apps/api/middleware.ts`**.
