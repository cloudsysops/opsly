...
23. **Siguiente** — p. ej. E2E invite (Playwright), más rutas bajo `/api/portal/tenant/[slug]/`, u operación VPS según `VISION.md`.
...
| OpenAPI (subset) | `docs/openapi-opsly-api.yaml` — portal: `/api/portal/me`, `/mode`, `/usage`, `/tenant/{slug}/me|mode|usage` (incr. 20, 2026-04-08); CI `validate-openapi` (incr. 21, 2026-04-05) |
...
22. **Siguiente capacidad Fase 4** — E2E invite, más handlers bajo `/api/portal/tenant/[slug]/`, o VPS según `VISION.md`.
...
- **`package.json`:** `npm run update-state`, `sync-agents` → `bash scripts/update-agents.sh`, `validate-context` → validación JSON local con `python3 -m json.tool`, **`validate-openapi`** → `node scripts/validate-openapi-yaml.mjs`.
