...
6. Instalar `scripts/install-git-pull-watcher.sh` en la Mac principal (y en PC-gamer una vez tenga el repo clonado) para que el pull de `main`/rama activa sea automático y el agente "despierte" cuando hay tarea nueva aquí — ver `docs/01-development/GIT-PULL-WATCHER.md` (PR #926). No reemplaza el flujo manual (`Al abrir una sesión nueva…` arriba); lo complementa manteniendo el repo al día sin que el humano tenga que acordarse de hacer `git pull`.
