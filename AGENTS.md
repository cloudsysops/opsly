...
8. **✅ Zero-Trust — `POST /api/portal/mode`** — *Hecho (2026-04-08).* Misma sesión verificada que `/api/portal/me`: `resolveTrustedPortalSession` antes de mutar `user_metadata.mode` (`apps/api/app/api/portal/mode/route.ts`). Tests ruta: `app/api/portal/__tests__/portal-routes.test.ts` (401, 403 sin slug, 400, 500, 200). Tests módulo compartido: `lib/__tests__/portal-trusted-identity.test.ts` (6 casos). Próximo: otras rutas bajo `/api/portal/*` o webhooks según prioridad.
...
8. **Ampliar Zero-Trust** a más rutas (p. ej. futuros handlers portal) — cuando se priorice; patrón: `resolveTrustedPortalSession` en `portal-trusted-identity.ts`.
...
| 2026-04-08 | `resolveTrustedPortalSession` en `portal-trusted-identity.ts` es la base común para feedback (vía `portal-feedback-auth`) y `POST /api/portal/mode`; tests dedicados en `portal-trusted-identity.test.ts` | Fase 4 incremento 8; una sola regla JWT+tenant+owner |
