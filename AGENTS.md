...
17. **Siguiente capacidad Fase 4** — más handlers bajo `/api/portal/tenant/[slug]/`, E2E invite o VPS según `VISION.md`.
...
| 2026-04-05 | Portal `fetchPortalTenant`: si `user_metadata.tenant_slug` existe → `GET /api/portal/tenant/{slug}/me`; si no → `GET /api/portal/me` (`tenantSlugFromUserMetadata`, `portal-server`, hooks) | Fase 4 incremento 17; alinea cliente con rutas Zero-Trust `[slug]` como `fetchPortalUsage` |
