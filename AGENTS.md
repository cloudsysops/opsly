...
20. **✅ OpenAPI — rutas portal `/usage` y `/tenant/{slug}/*`** — *Hecho (2026-04-08).* `docs/openapi-opsly-api.yaml`: `GET /api/portal/usage`; `GET /api/portal/tenant/{slug}/me`; `POST /api/portal/tenant/{slug}/mode`; `GET /api/portal/tenant/{slug}/usage` (query `period`); alineado a implementación y a `portal-api-paths`. `docs/SECURITY_CHECKLIST.md` (referencia contrato en portal cliente). Sin cambio de runtime.
21. **✅ CI — validación OpenAPI YAML** — *Hecho (2026-04-05).* `scripts/validate-openapi-yaml.mjs`: parse con paquete `yaml` (devDependency raíz), comprobación de `openapi` y `paths`. `npm run validate-openapi`. `.github/workflows/validate-context.yml`: paso tras `npm ci`. Sin cambio de runtime.
22. **✅ Portal — Vitest validación formulario invite (`/invite/[token]`)** — *Hecho (2026-04-04).* `lib/invite-activation-validation.ts` (`validateInviteActivationForm`, `inviteActivationErrorMessage`); `lib/__tests__/invite-activation-validation.test.ts` (6 casos); `app/invite/[token]/invite-activate.tsx` delega en el módulo (mismos mensajes ES). Sin Supabase en tests. Portal **19** tests Vitest; API **155** sin cambio.
23. **✅ CI — OpenAPI paths portal obligatorios** — *Hecho (2026-04-08).* `scripts/validate-openapi-yaml.mjs` (`REQUIRED_PORTAL_PATHS`): exige en `paths` las 6 rutas `/api/portal/me`, `/api/portal/mode`, `/api/portal/usage`, `/api/portal/tenant/{slug}/me|mode|usage` (alineado a incr. 20). `docs/SECURITY_CHECKLIST.md` referencia la validación. Sin cambio de runtime.
24. **✅ OpenAPI — `/api/feedback` (POST portal JWT + GET admin)** — *Hecho (2026-04-08).* `docs/openapi-opsly-api.yaml`: `POST /api/feedback` (cuerpo `message` + opcionales alineados a `parseFeedbackPostFields`); `GET /api/feedback` (`status`, `limit`; admin `Bearer` / `x-admin-token`). `validate-openapi-yaml.mjs` (`REQUIRED_FEEDBACK_PATHS`) exige `/api/feedback`. Sin cambio de runtime. API **155** tests sin regresión.
25. **Siguiente** — p. ej. E2E invite (Playwright), más rutas bajo `/api/portal/tenant/[slug]/`, u operación VPS según `VISION.md`.
...
| OpenAPI (subset) | `docs/openapi-opsly-api.yaml` — portal (incr. 20) + **`/api/feedback`** GET/POST (incr. 24); CI `validate-openapi`: parse + **portal + feedback** paths obligatorios (incr. 21, 23, 24) |
...
24. **Siguiente capacidad Fase 4** — E2E invite (Playwright), más handlers bajo `/api/portal/tenant/[slug]/`, o VPS según `VISION.md`.
...
- **`package.json`:** `npm run update-state`, `sync-agents` → `bash scripts/update-agents.sh`, `validate-context` → validación JSON local con `python3 -m json.tool`, **`validate-openapi`** → `node scripts/validate-openapi-yaml.mjs` (parse + `paths` + portal + `/api/feedback` obligatorios, incr. 21–24).
