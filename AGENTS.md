...
25. **✅ Portal — health API + `portal-api-paths` + Playwright E2E smoke** — *Hecho (2026-04-08).* API: `GET /api/portal/health?slug=` (público, monitoring); `GET /api/portal/tenant/{slug}/health` (JWT + `tenantSlugMatchesSession`); `lib/portal-health-json.ts` (`respondPortalTenantHealth`). Cliente: `portalHealthUrl`, `portalPublicHealthUrl`, `fetchPortalHealth` (`lib/tenant.ts`). **`@playwright/test`**, `playwright.config.ts`, `e2e/portal.spec.ts` (login, invite, redirects dashboard). OpenAPI + `REQUIRED_PORTAL_PATHS` incluyen ambas rutas health. Tests API **162**; portal Vitest **21**; E2E opcional `npm run test:e2e --workspace=@intcloudsysops/portal`.
...
25. **Siguiente capacidad Fase 4** — E2E invite con credenciales en CI, más handlers bajo `/api/portal/tenant/[slug]/`, o VPS según `VISION.md`.
...
| 2026-04-08 | Portal health: `GET /api/portal/health?slug=` público + `GET /api/portal/tenant/{slug}/health` Zero-Trust; `portalPublicHealthUrl` / `portalHealthUrl`; Playwright smoke en `apps/portal/e2e` | Fase 4 incremento 25; monitoring + E2E sin romper API |
