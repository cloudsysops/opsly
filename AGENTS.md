...
13. **✅ Portal — métricas LLM vía ruta con `[slug]`** — *Hecho (2026-04-09).* `fetchPortalUsage(token, period, tenantSlug)` en `apps/portal/lib/tenant.ts` → `GET /api/portal/tenant/{slug}/usage`; `requirePortalPayloadWithUsage` pasa `payload.slug` tras `GET /api/portal/me` (mismo JWT; el path refuerza alineación sesión–slug). Sin `tenantSlug` opcional sigue existiendo `GET /api/portal/usage`. Validación: `npm run type-check` + `lint` portal; API **152** tests (suite actual).
14. **✅ `GET /api/portal/tenant/[slug]/me` + `respondTrustedPortalMe`** — *Hecho (2026-04-09).* `lib/portal-me-json.ts`: respuesta JSON compartida con **`GET /api/portal/me`**; `app/api/portal/me/route.ts` delega en **`respondTrustedPortalMe`**. **`GET /api/portal/tenant/[slug]/me`:** `tenantSlugMatchesSession` → **403** si el slug no coincide. Tests: `portal-routes.test.ts` (401, 403, 200). `docs/SECURITY_CHECKLIST.md`. Cliente portal no cambia (`fetchPortalTenant` → `/api/portal/me`).
15. **✅ `POST /api/portal/tenant/[slug]/mode` + `applyPortalModeUpdate`** — *Hecho (2026-04-08).* `lib/portal-mode-update.ts`: mutación de **`user_metadata.mode`** compartida con **`POST /api/portal/mode`**; `app/api/portal/mode/route.ts` delega en **`applyPortalModeUpdate`**. **`POST /api/portal/tenant/[slug]/mode`:** `tenantSlugMatchesSession` → **403** si el slug no coincide (sin llamar a Supabase admin). Tests: `portal-routes.test.ts` (401, 403 sin `updateUserById`, 200). `docs/SECURITY_CHECKLIST.md`. Cliente portal sigue en **`POST /api/portal/mode`**.
16. **Siguiente** — p. ej. más rutas bajo `/api/portal/tenant/[slug]/`, E2E invite, u operación VPS según `VISION.md`.
...
15. **Siguiente capacidad Fase 4** — más handlers bajo `/api/portal/tenant/[slug]/`, E2E invite o VPS según `VISION.md`.
...
| 2026-04-09 | `GET /api/portal/tenant/[slug]/me`: mismo patrón que usage; `respondTrustedPortalMe` comparte cuerpo con `GET /api/portal/me` | Fase 4 incremento 14; cliente portal sigue usando `/api/portal/me` |
| 2026-04-08 | `POST /api/portal/tenant/[slug]/mode`: `applyPortalModeUpdate` comparte lógica con `POST /api/portal/mode`; slug de ruta validado antes de Supabase admin | Fase 4 incremento 15; cliente portal sigue usando `/api/portal/mode` |
