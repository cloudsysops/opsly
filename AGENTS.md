...
6. Instalar `scripts/install-git-pull-watcher.sh` en **las tres máquinas** (y en cualquiera que se sume después) para que el pull sea automático y el agente "despierte" cuando hay tarea nueva aquí — ver `docs/01-development/GIT-PULL-WATCHER.md` (PR #926). No reemplaza el flujo manual (`Al abrir una sesión nueva…` arriba); lo complementa manteniendo el repo al día sin que el humano tenga que acordarse de hacer `git pull`.
   - **Mac principal (`opsly-admin`):** `./scripts/install-git-pull-watcher.sh` (LaunchAgent).
   - **VPS (`vps-dragon`, `/opt/opsly`):** `sudo ./scripts/install-git-pull-watcher.sh` (systemd system).
   - **PC-gamer (`opsly-worker`) y cualquier máquina futura:** clonar el repo en `~/opsly` y correr `./scripts/install-git-pull-watcher.sh --user` (systemd user, sin sudo; la unidad es genérica vía `%h`, no requiere editar nada por host).
7. **GPU de PC-gamer para Ollama (2026-08-07, PR #926):** el usuario confirmó que el objetivo prioritario es poder enviarle tareas de inferencia local a PC-gamer aprovechando su GPU. Esto **ya es una decisión aceptada** — [`ADR-024`](docs/adr/ADR-024-ollama-local-worker-primary.md) ("Ollama Local como Provider Primary en Worker Remoto") — validada en producción con el worker anterior; PC-gamer solo reemplaza el hardware, no la arquitectura. **No crear un ADR nuevo.** Pasos concretos, en orden:
   - **7.1 Prerrequisito (en PC-gamer, antes de todo):** driver NVIDIA + [NVIDIA Container Toolkit](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/install-guide.html) instalados. Verificar con `nvidia-smi` funcionando *fuera* de Docker antes de continuar.
   - **7.2 Compose con GPU:** ya existe `infra/docker-compose.opslyquantum.gpu.yml` (override, NO modifica `docker-compose.opslyquantum.yml` — así el compose base sigue funcionando sin GPU en Mac/VPS/futuros workers CPU-only). Levantar con: `OLLAMA_GPU=1 ./scripts/ensure-ollama-local.sh --ensure` (ya soporta la variable, ver PR #926) o directamente `docker compose -f infra/docker-compose.opslyquantum.yml -f infra/docker-compose.opslyquantum.gpu.yml --env-file infra/opslyquantum.env up -d ollama`.
   - **7.3 systemd con GPU:** usar `infra/systemd/opsly-ollama.gpu.service` (variante nueva de `opsly-ollama.service`, no lo reemplaza) — `sudo cp infra/systemd/opsly-ollama.gpu.service /etc/systemd/system/opsly-ollama.service && sudo systemctl daemon-reload && sudo systemctl enable --now opsly-ollama`.
   - **7.4 Verificar GPU en uso:** `docker exec opslyquantum-ollama nvidia-smi` y, tras correr un modelo, `docker exec opslyquantum-ollama ollama ps` → columna `PROCESSOR` debe decir `100% GPU` (si dice `CPU`, el passthrough no está funcionando — revisar 7.1).
   - **7.5 Conectar al control plane (VPS):** actualizar `OLLAMA_URL` en Doppler `prd` (`ops-intcloudsysops`) apuntando a `http://<ip-tailscale-pc-gamer>:11434` (mismo patrón que ADR-024, variable ya existe — **no hardcodear la IP en código ni en chats**, solo en Doppler). Elegir `OLLAMA_MODEL` según VRAM de la GPU (más VRAM → modelo más grande; ver tabla de ejemplo en ADR-024).
   - **7.6 Validar end-to-end:** seguir el checklist de implementación al final de ADR-024 (`POST /api/admin/ollama-demo`, health daemon `llama_local` OK, `npm run type-check`). **No** es necesario reescribir `scripts/adr-024-diagnose.sh` (está fechado/hardcodeado a la IP del worker anterior) — solo actualizar la IP en los comandos SSH al ejecutar manualmente, o pedir que se genere una versión nueva si hace falta automatizarlo de verdad.
   - **Selección de worker por tarea:** ya funciona sin cambios adicionales — `lib/runtime/worker-selector.ts` tiene el `WorkerType` `'ollama'` genérico (no específico de host) y el orchestrator (`apps/orchestrator/src/workers/OllamaWorker.ts`) ya enruta jobs tipo `ollama` al `OLLAMA_URL` configurado. Una vez el paso 7.5 esté hecho, cualquier tarea que la orquestación clasifique como `cheap`/`decision`+esfuerzo `S` cae en PC-gamer automáticamente (routing ya implementado, `HERMES_LOCAL_LLM_FIRST=true`).

**Sobre "conectar todas las máquinas por Docker" (respuesta a la pregunta del usuario):** Docker **no** es el conector entre máquinas aquí — cada host corre sus propios contenedores localmente (`infra/docker-compose.opslyquantum.yml` en el worker, compose equivalente en VPS), y el conector real entre máquinas es **Tailscale** (red) + **Redis/BullMQ** (cola de trabajos compartida) + **LLM Gateway** (routing), ya construido y validado en ADR-024. Docker Swarm/overlay networks entre hosts está explícitamente prohibido sin ADR (`AGENTS.md` § Roadmap Vivo, "No negociar"). No hace falta ni se debe intentar eso — el sistema ya está "sincronizado" vía Tailscale+Redis; lo que faltaba (y ahora está documentado arriba) era darle GPU real a Ollama en el nuevo host.
