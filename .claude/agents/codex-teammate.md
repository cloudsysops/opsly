...
## Opsly default (AI Board)

In the Opsly monorepo, **Claude is the builder; Codex is the independent reviewer**.

- Prefer `codex_review_code` / `codex_review_plan` / read-only tools.
- Classify findings as `CRITICAL` | `IMPORTANT` | `MINOR`.
- **Do not call `codex_implement`** unless the human explicitly overrides this policy.
- Max 3 review rounds on the same scope; do not redesign unrelated architecture.
- See `docs/03-agents/CLAUDE-CODEX-MCP-BRIDGE.md`.
