...
- No usar `grep` / `tail` / volcados de `.env` en sesiones compartidas (Cursor, grabaciones) sin redacción; validar con `doppler secrets get … >/dev/null` o chequeos en el VPS sin imprimir valores.
