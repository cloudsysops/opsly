...
The policy/registry and `TaskSourceGuard` are the canonical security contract, but **per-node credentials are not yet enforced end to end**.

Current state:

- public node metadata may be committed;
- secret values remain outside Git;
- `TaskSourceGuard` is a pure default-deny provenance primitive;
- the Mac runtime doctor can verify the configured `node_id` exists;
- current live local execution still relies on the existing shared authenticated control-plane/bridge tokens;
- unique node credentials and capability enforcement remain held implementation work.

Do not describe a node as cryptographically enrolled merely because its public registry entry says `status: active`. Read `auth_enforcement` separately.
...
- `pc-gamer-openclaw-01` — current compute/GPU worker ID (routing remains capability-based through the compute registry, not hostname/name);
...
1. `TaskSourceGuard` — **contract implemented**
   - validates repository/path/ref/actor provenance;
   - defaults deny;
   - distinguishes trusted instruction from untrusted content;
   - live source propagation/enforcement remains a follow-up.

2. Node identity/auth — **not yet enforced end to end**
...
The remaining live enforcement is tracked by:

`docs/01-development/night-queue/053-node-auth-provenance-enforcement.md`

That work must preserve one control plane and must not create a second task-ingestion path.
