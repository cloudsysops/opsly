...
**Base en main:** [#922](https://github.com/cloudsysops/opsly/pull/922). Night-shift hardening: rama `feat/opsly-moon-night-shift`.

## Qué es

Control plane profesional multi-tenant: clientes, blueprints, módulos, agentes, colas, approvals, automatizaciones, health, usage/costos etiquetados, ventures (si existen en config), command center dry-run, reportes índice.
...
- **No** usa AgentTaskEnvelopeV1 (contrato inexistente).

## Documentación

| Doc | Rol |
| --- | --- |
| [OPSLY-MOON-AUDIT.md](./OPSLY-MOON-AUDIT.md) | Auditoría madurez |
...
- Shell: `MoonShell` / `MoonSidebar` / `MoonHeader` / `MoonCommandBar`
- Tests: `npm run test:moon --workspace=@intcloudsysops/admin`
...
4. LLM solo vía Orchestrator → Gateway (Command Center no llama LLM en v1).
