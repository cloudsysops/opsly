...
## Ramas fijas por ambiente y módulo

La convención queda separada en **ramas fijas** (larga vida, protegidas) y **ramas de trabajo** (temporales, una tarea por agente). El objetivo es que los agentes sepan dónde integrar sin convertir `main` en un punto de choque.
...
| `staging` | Integración pre-prod | Sí: staging (`:staging`) | Revisor de integración | Recibe PRs desde ramas `module/*` o hotfixes. |

> No crear `develop`: `staging` cumple ese rol y ya está conectado al workflow de deploy del VPS.

### Módulos (integración por dominio)

Estas ramas son buffers de integración por área. **No despliegan directo** y no deben acumular semanas sin PR hacia `staging`.

| Rama fija | Ámbito principal | Rutas típicas |
| --------- | ---------------- | ------------- |
...
```text
<tipo>/<modulo>/<YYYYMMDD>-<tema-corto>
```
...
- Ejemplos: `feat/portal/20260508-usage-card`, `fix/api/20260508-feedback-auth`, `docs/infra/20260508-vps-runbook`.

### Cadena de promoción

```text
feat|fix|docs/<modulo>/<fecha>-<tema>
  → module/<modulo>
  → staging
  → main
```
...
1. El agente crea su rama temporal desde `origin/module/<modulo>` si existe; si no existe todavía, desde `origin/staging`.
2. Una tarea que toca varios módulos debe elegir **un módulo owner** y mencionar módulos secundarios en el PR. Si el cambio es grande, dividir en PRs por módulo.
3. `module/*` se sincroniza con `staging` al menos una vez por semana o antes de probar E2E.
4. `hotfix/*` sale desde `main`, entra a `main`, y luego se retropropaga a `staging` y al `module/*` afectado.
5. Tras mergear una rama temporal, borrar rama remota y local. Las únicas ramas persistentes son `main`, `staging` y `module/*`.
...
Después, configurar protección de ramas en GitHub para `main`, `staging` y `module/*` (PR requerido, checks requeridos, sin force-push).
...
2. **Nueva capacidad o fix:** rama temporal desde `module/<modulo>` actualizado (o `staging` si el módulo todavía no existe):
   - `git fetch origin && git checkout module/<modulo> && git pull --ff-only origin module/<modulo>`
   - `git checkout -b feat/<modulo>/<YYYYMMDD>-<tema-corto>` o `fix/<modulo>/<YYYYMMDD>-<ticket>`.
   - Si es hotfix productivo: partir desde `main` con `hotfix/<modulo>/<YYYYMMDD>-<tema>`.
3. **Integración:** **Pull Request** hacia `module/<modulo>`; después promoción `module/<modulo>` → `staging` → `main` con CI verde.
...
1. **Arranque:** `module/<modulo>` al día (`fetch` + `pull --ff-only`; fallback `staging`). Crear rama temporal `feat/<modulo>/…` o `fix/<modulo>/…` (preferible a depender solo de `cursor/*` generadas por la herramienta: renombrar o abrir PR desde esa rama y tratarla como temporal).
...
4. **PR:** abrir PR hacia `module/<modulo>`, descripción breve (qué / por qué / cómo validar). Para promoción, abrir PR `module/<modulo>` → `staging` y luego `staging` → `main`.
...
| Situación | Acción |
|-----------|--------|
| El trabajo **debe** entrar en producto | Rebase o merge de `module/<modulo>` (o `staging` para promoción) en la rama, `push`, PR, merge, borrar rama temporal. |
...
| **Ramas remotas** | `git fetch origin --prune` y `git branch -r` | Solo ramas fijas (`origin/main`, `origin/staging`, `origin/module/*`) y temporales con PR abierto; borrar lo demás. |
