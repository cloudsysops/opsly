...
| H3b | **40 of 132 migrations are not re-runnable** — re-applying the chain over an already-migrated database fails with `policy already exists` (26), `relation already exists` (12) and six others. Compounds with H3: a half-applied non-atomic migration **cannot be repaired by re-running it**, because the statements that succeeded now abort it at a different point. Recovery needs a human reading SQL. | Measured (`replay.sh --twice`) |
...
| 6 | Migration replay green | **NO** — 4 migrations cannot apply from clean, and 40 of 132 are not re-runnable. Measured, documented, not yet fixed (2 of the 4 are franchise-owned) |
