...
> **Canónico (infra):** editar este archivo. Stub de compatibilidad: [`docs/TENANT-PRODUCTION-BASELINE.md`](../TENANT-PRODUCTION-BASELINE.md).
...
| 3 | `POST /api/tenants` sin persistencia (histórico) | Tras deploy API reciente, validar persistencia y logs en prod. |
| 4 | Rutas suspend/resume sin auth en web legado | Proxy fuerza flujo API con control admin. |
| 5 | `v1/keys` solo con `x-tenant-id` | Rotación y endurecimiento: [TENANT-PRODUCTION-HARDENING.md](./TENANT-PRODUCTION-HARDENING.md). |
...
- [TENANT-PRODUCTION-CHECKLIST.md](../runbooks/TENANT-PRODUCTION-CHECKLIST.md) — criterios por tenant.
