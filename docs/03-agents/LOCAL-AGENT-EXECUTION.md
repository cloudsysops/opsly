...
Monitors `.cursor/prompts/queue/` for pending `.md` files and automatically submits them to the orchestrator.
...
**Features:**
- Watches `.cursor/prompts/queue/*.md` for changes
- Processes existing `status: pending` prompts on startup
- Reads `docs/01-development/ACTIVE-PROMPT.md` as operational context
- Parses YAML frontmatter
- Auto-submits to orchestrator
- Polls `/api/job-status/{job_id}`
- Appends `## Respuesta agente (...)` to the same prompt file
- Tracks state in `.cursor/prompts/queue/.metadata.json`
...
mkdir -p .cursor/prompts/queue
cat > .cursor/prompts/queue/my-task.md << 'EOF'
---
status: pending
agent_role: executor
max_steps: 5
...
# 4. Append "Respuesta agente" when the job completes
