...
export function analyzeAutonomousContext(query: string): AutonomyContext {
  ...
  const requiresConfirmation = confidence < 0.3 || domain === 'unknown' || intent === 'deploy';
...
export function loadSkillContent(
  skillName: string
): { name: string; content: string; manifest: object | null } | null {
...
export async function autonomousExecute(query: string): Promise<{
  ...
  const skills = plan.map((s) => loadSkillContent(s)).filter(Boolean) as ReturnType<
    typeof loadSkillContent
  >[];
