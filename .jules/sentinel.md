...
## 2026-07-04 - [Missing Rate Limiting and Audit Logging in Governance Consent API]
**Vulnerability:** The `POST /api/governance/consent` endpoint was exposed without any rate limiting or audit logging. This allowed for potential mass-creation of consent records (PII-linked compliance data) without a traceable history of these sensitive actions.
**Learning:** Even when endpoints handle sensitive PII or compliance data, they might be overlooked for perimeter protections if they are perceived as "simple" data capture routes. Consistency in applying `checkRateLimit` and `logAuditEvent` is essential across all public mutation endpoints.
**Prevention:** Mandate the use of security tests (e.g., `security.test.ts`) for all new public-facing endpoints to verify the presence of rate limiting and audit logging. Use the 50-line ESLint rule as a trigger to decompose route handlers into secure, testable units.
