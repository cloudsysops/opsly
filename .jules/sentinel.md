...
## 2026-07-20 - [Missing Rate Limiting and Audit Logging in Governance Consent API]
**Vulnerability:** The public unauthenticated `POST /api/governance/consent` endpoint lacked rate limiting and security audit logging. This allowed for potential mass-spamming or database/resource exhaustion from automated scripts and lacked compliance traceability for user consent tracking.
**Learning:** Compliance-related and other public endpoints are prone to being implemented without applying available perimeter protections, even when standard helpers like `checkRateLimit` and `logAuditEvent` exist.
**Prevention:** Systematically apply `checkRateLimit` and `logAuditEvent` to all public, state-changing endpoints in the `/api/governance` suite and co-locate security tests to verify their integration.
