...
## 2026-07-02 - [Missing Rate Limiting and Audit Logging in Governance Breach API]
**Vulnerability:** The `POST /api/governance/breach` endpoint was protected only by a shared secret and lacked IP-based rate limiting and security audit logging. This made it vulnerable to brute force on the secret and lacked a traceable history for these critical security events.
**Learning:** Even internal-only endpoints protected by shared secrets require defense-in-depth measures like rate limiting and audit logging to prevent abuse and ensure non-repudiation.
**Prevention:** Mandate the use of `checkRateLimit` and `logAuditEvent` for all mutation endpoints (`POST/PATCH/DELETE`), regardless of their intended audience (internal vs public).
