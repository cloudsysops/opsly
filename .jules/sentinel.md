...
## 2026-07-30 - [Missing Rate Limiting and Audit Logging in Peskids Public Form Retrieval Endpoint]
**Vulnerability:** The public `GET /api/peskids/forms/[formId]` route was completely exposed without client IP-based rate limiting or security audit logging. This created a vector for bulk form schema scraping and server resource exhaustion.
**Learning:** Public metadata GET endpoints are often overlooked during security hardening because they do not mutate state. However, they are highly vulnerable to systematic scraping, schema harvesting, and Denial-of-Service attacks.
**Prevention:** Extend rate limiting (`checkRateLimit`) and audit logging (`log_audit_event`) to all public endpoints, regardless of whether they perform write actions or merely serve metadata/definitions.
