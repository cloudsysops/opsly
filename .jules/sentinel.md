...
## 2026-07-21 - [Missing Rate Limiting and Audit Logging in DSAR Token Verification]
**Vulnerability:** The `GET /api/governance/dsar/[token]` endpoint exposed sensitive compliance and PII data without rate limiting or security audit logging. This created a token-scanning/brute-force vulnerability and lacked traceability for access to sensitive compliance records.
**Learning:** Security controls like rate limiting and auditing applied to resource creation (POST) must consistently be extended to associated status check or verification endpoints (GET/PATCH) that interact with the same sensitive compliance data.
**Prevention:** Always secure token verification or status check endpoints with IP-based rate limiting (e.g., `dsar-verify:${ip}`) and explicit audit logging with appropriate actions (e.g., `VERIFY`). Ensure co-located security tests verify these boundary protections.
