...
## 2026-07-22 - [Missing Rate Limiting and Auditing on DSAR Token Verification]

**Vulnerability:** The `GET /api/governance/dsar/[token]` endpoint was exposed without any rate limiting or audit logging. This allowed attackers to perform brute-force token enumeration or denial-of-service style scans on sensitive DSAR verification tokens, and omitted a compliance audit history.
**Learning:** Security controls like rate limiting and auditing applied to resource creation (such as `POST /api/governance/dsar`) are often not consistently extended to associated retrieval, status check, or verification endpoints (such as `GET/PATCH`). These associated endpoints interact with the exact same sensitive compliance data and PII, posing equivalent risks of enumeration or tracing gaps.
**Prevention:** Always apply standard IP-based rate limiting (`checkRateLimit`) and verification/read audit logging (`logAuditEvent`) to endpoints that process verification tokens or display status of compliance/PII actions. Include specific verification-phase tests to cover brute force vectors on tokens.
