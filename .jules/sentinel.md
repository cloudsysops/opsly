...
## 2026-07-02 - [Missing Rate Limiting and Audit Logging in Shield Alert Config]
**Vulnerability:** The `POST /api/shield/alerts/config` endpoint lacked IP-based rate limiting and security audit logging, despite managing sensitive security configurations for tenants.
**Learning:** Security-focused modules (like Shield) can ironically miss standard perimeter protections if they are perceived as inherently "secure" due to their purpose or if auth/metering are already present. Rate limiting must be at the very entry point.
**Prevention:** Always verify that every POST handler in `apps/api` (even those behind JWT auth) implements `checkRateLimit` using a specific key (e.g., `shield-alert-config:${ip}`) and `logAuditEvent` for state-changing operations.
