...
## 2026-07-17 - [Missing Rate Limiting and Audit Logging in Defense Platform API]
**Vulnerability:** The administrative POST endpoints under `apps/api/app/api/defense/` (`/audits` and `vulnerabilities/[id]/remediate`) lacked IP-based rate limiting and security audit logging. An authorized but compromised admin or abusive account could schedule infinite audits or falsify remediation states without leaving a traceable log trail or triggering rate limiting controls.
**Learning:** Even highly privileged admin or platform-level modules might omit secondary perimeter checks under the assumption that "only admins can access them," but proper defense-in-depth requires every state-changing route to be throttled and audited.
**Prevention:** Mandate that all administrative mutating endpoints (especially under `apps/api/app/api/defense/`) implement rate limits and log a security audit event on success. Write co-located security tests using relative traversals in a `__tests__` folder to verify these protections.
