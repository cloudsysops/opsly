...
## 2026-07-01 - [Missing Rate Limiting and Audit Logging in Breach API]
**Vulnerability:** The `POST /api/governance/breach` endpoint was exposed with authentication but lacked rate limiting and centralized audit logging.
**Learning:** Endpoints protected by service-role secrets still require perimeter defense (rate limiting) and audit trails to maintain "defense in depth" and ensure accountability for sensitive operations.
**Prevention:** Always apply `checkRateLimit` and `logAuditEvent` to all mutation endpoints, even those that are "internal" or authenticated via secrets.
