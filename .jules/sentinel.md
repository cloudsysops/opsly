...
## 2026-06-30 - [Missing Rate Limiting and Security Audit Logging in Governance Consent API]
**Vulnerability:** The public `POST /api/governance/consent` endpoint was exposed without any rate limiting or security audit logging. This could allow for consent spam or automated database insertion of fake consent records with no traceability/audit trail.
**Learning:** Compliance-focused endpoints (such as those under `/api/governance/`) might be designed assuming their inputs are low-risk, but public-facing inputs require rate limiting and non-repudiation logging to satisfy security and data governance standards.
**Prevention:** Protect public-facing compliance and data manipulation endpoints using standard IP-based rate limiting (`checkRateLimit`) and security audit logging (`logAuditEvent`) with dynamic context metadata. Include dedicated security verification tests in workspace pipelines.
