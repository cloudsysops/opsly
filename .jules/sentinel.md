...
## 2026-07-28 - [Missing Rate Limiting and Audit Logging in DSAR Verification Endpoint]
**Vulnerability:** The public `GET /api/governance/dsar/[token]` endpoint was exposed to token harvesting/brute forcing because it did not implement rate limiting or security audit logging.
**Learning:** Public token-verification or status-check endpoints (which are often accessed via dynamic route parameters) are occasionally overlooked when securing POST requests. They must also be protected with rate limiting to prevent enumeration/brute force and audit logged for complete visibility.
**Prevention:** Always implement IP-based rate limiting (e.g. `dsar-verify:${ip}`) and secure audit logging on any public endpoints, including GET status-checks or verification token handlers. Avoid logging raw credentials/tokens in audit log metadata to protect sensitive PII.
