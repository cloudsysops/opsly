...
## 2026-07-31 - [Missing Rate Limiting and Audit Logging in Growth Outreach API]

**Vulnerability:** The GET and POST endpoints for `/api/growth/outreach-template` were missing client IP-based rate limiting and security audit logging. Although they required `requireAdminAccess`, they still exposed highly personalized template generation containing sensitive contact PII (emails, names) without trace trails or perimeter limits, risking automated data extraction.
**Learning:** Administrative endpoints that handle/generate communications with sensitive PII (like outreach emails) are often overlooked because they are behind authentication wrappers. However, defense-in-depth requires active audit logs and rate-limits to trace accesses and prevent volume-based leaks.
**Prevention:** Always implement standard `checkRateLimit` (with a prefix like `growth-outreach:${ip}`) and `logAuditEvent` (with recipient email masked using custom `maskEmail`) in any administrative outreach or growth template route handler. Ensure corresponding security tests cover 429, 400, and successful masked audit logging scenarios.
