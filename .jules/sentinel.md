...
## 2026-07-15 - [Missing Security Controls in Governance Consent API]
**Vulnerability:** The `POST /api/governance/consent` endpoint lacked IP-based rate limiting and security audit logging, making it vulnerable to automated abuse and lacking a traceable record of compliance actions.
**Learning:** New functional routes in the governance module were implemented with input validation but without the perimeter security patterns (rate limiting and auditing) established in the DSAR creation route.
**Prevention:** Systematically apply `checkRateLimit` and `logAuditEvent` to all public-facing mutation endpoints. Use automated security tests to verify the presence of these controls for any new route in the `governance` directory.
