...
## 2026-07-26 - [Missing Rate Limiting and Auditing on Public Form Retrieval]
**Vulnerability:** The public form retrieval endpoint (`GET /api/peskids/forms/[formId]`) was exposed without any rate limiting or security audit logging. This made it vulnerable to automated scraping, form discovery, and Denial of Service (DoS) attacks.
**Learning:** Security perimeter controls like rate limiting and auditing are often applied to creation/submission actions (such as form POST submission) but overlooked on retrieval actions (form GET). However, retrieval endpoints are equally attractive targets for scrapers looking to harvest form structures or exhaust server resources.
**Prevention:** Systematically extend rate-limiting (IP-based keys like `peskids-form-get:${ip}`) and audit logging (action: `form_retrieved`) to public-facing GET endpoints that serve sensitive metadata or form schemas. Use co-located security tests to ensure continuous coverage.
