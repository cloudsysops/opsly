...
## 2026-07-28 - [Missing Auditing and Code Duplication in Status Check API]
**Vulnerability:** The public tenant status check endpoint (`/api/public/tenants/status`) queried tenant onboarding and layouts with user emails (PII) but lacked security audit logging. It also bypassed the standard centralized rate limiter in favor of a duplicated custom Redis/Lua script block, which bypassed the local/memory rate limiting fallback mechanisms.
**Learning:** Status-check (GET) endpoints that retrieve records based on sensitive identifiers like user email addresses present verification and data-probing risks. Code duplication of rate-limiting logic is prone to missing fallback and secure default patterns.
**Prevention:** Standardize all unauthenticated or status-check endpoints with centralized rate limiting and security audit logs (`logAuditEvent`). Always mask PII fields (like user emails) in log metadata to maintain trace logs while preventing data leakage.
