...
## 2026-07-19 - [Missing Rate Limiting and Audit Logging in Voice API Proxy Endpoints]
**Vulnerability:** Voice communication endpoints (`POST /api/voice/calls`, `PATCH /api/voice/calls/[callId]`, and `POST /api/voice/messages/voice`) which proxy sensitive telephony and messaging operations to the backend orchestrator were exposed without any rate limiting or audit logging. This risked automated telephony abuse, carrier charges, resource exhaustion, and lacked traceability for administrative actions.
**Learning:** Endpoints that act as direct proxies to downstream orchestration services are sometimes treated purely as "pass-throughs" and left without the API gateway's perimeter protections (rate limiting and audit logging), assuming the downstream service handles them.
**Prevention:** Always apply the standard defensive perimeter triad (`requireAdminAccess`, `checkRateLimit`, and `logAuditEvent`) to any administrative pass-through or proxy routes that handle state mutations, heavy file uploads, or high-cost system tasks.
