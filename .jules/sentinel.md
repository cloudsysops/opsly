...
## 2026-07-25 - [Missing Rate Limiting and Audit Logging on DSAR Verification GET Route]

**Vulnerability:** The DSAR token verification endpoint `GET /api/governance/dsar/[token]` was exposed without any rate limiting or security audit logging. This made it vulnerable to brute-force token harvesting and lacked tracking for sensitive compliance-related state changes.
**Learning:** Security controls applied to resource creation (POST) are sometimes not consistently extended to associated verification/retrieval (GET/PATCH) handlers, leaving sensitive data vulnerable to abuse.
**Prevention:** Always extend IP-based rate limiting and standard audit logging (e.g., action: `VERIFY`) to all endpoints dealing with sensitive PII or compliance actions, including token verification routes.
