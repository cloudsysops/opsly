...
## 2026-07-29 - [Missing Rate Limiting and Audit Logging on Public Bookings Endpoint]
**Vulnerability:** The public booking creation endpoint `/api/local-services/public/tenants/[slug]/bookings` permitted unlimited requests without client IP rate limiting or any audit logging, posing spam/denial-of-service and compliance traceability risks.
**Learning:** Security controls (rate limiting, audit logs) implemented for other public modules (e.g. Peskids) were overlooked during the development of local-services endpoints.
**Prevention:** Ensure all public-facing mutation handlers (e.g., POST/PATCH/DELETE) systematically employ the centralized `checkRateLimit` and `logAuditEvent` utilities on entry.
