...
## 2026-07-10 - [Missing Rate Limiting and Audit Logging in Internal Help Request API]
**Vulnerability:** The POST and PATCH endpoints for /api/internal/help-request lacked rate limiting and audit logging, making them susceptible to automated abuse and lacking a traceable history for sensitive support actions.
**Learning:** Internal-facing support tools (like help-request) are often overlooked for perimeter defense because they require admin authentication. However, defense-in-depth requires that even authenticated mutation endpoints have rate limits and audit trails to mitigate compromised credentials or insider threats.
**Prevention:** Mandate the use of checkRateLimit and logAuditEvent for all POST/PATCH/DELETE handlers, regardless of their "internal" or "external" classification.
