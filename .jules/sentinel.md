...
## 2026-07-31 - [Missing Rate Limiting and Audit Logging in Local-Services Public Endpoints]
**Vulnerability:** Public "local-services" endpoints for bookings and available slot grids (POST and GET under `/api/local-services/public/tenants/[slug]/`) lacked IP-based rate limiting and security audit logging. This made them vulnerable to automated brute-forcing of available slots, slot extraction, and reservation database flooding without any traceability. Additionally, raw customer emails were stored in plaintext logs in other parts of the system, posing a risk of PII leakage.
**Learning:** Rapidly developed customer-facing and technician scheduling public APIs often lack standard rate limiting and audit logging wrappers. This exposes the backend database to automated floods. Moreover, logging raw email addresses directly violates PII storage security best practices.
**Prevention:** Always secure all public, unauthenticated routes with IP-based rate limiting keys (e.g. `local-services-booking:${ip}`). Implement a secure masking function (e.g. `maskEmail`) to hide PII values before appending them to audit log metadata. Co-locate dedicated unit tests verifying rate limiting 429 status and log presence under route test directories.
