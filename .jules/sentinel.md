...
## 2026-07-06 - [Missing Rate Limiting and Audit Logging in Consent API]
**Vulnerability:** The public-facing `POST /api/governance/consent` endpoint lacked IP-based rate limiting and centralized audit logging. This exposed the system to automated spamming of consent records and lacked a traceable security trail for sensitive legal compliance actions.
**Learning:** Compliance-heavy endpoints (like those for recording user consent) are often assumed to be protected because they involve database writes, but without perimeter defense (rate limiting) and non-repudiation (audit logs), they remain vulnerable and difficult to audit during security incidents.
**Prevention:** Mandate the use of `checkRateLimit` and `logAuditEvent` for all public mutation routes in the `governance` module. Ensure security tests verify these protections for every new compliance-related endpoint.
