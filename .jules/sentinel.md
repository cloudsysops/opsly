...
## 2026-07-27 - [Missing Rate Limiting and Audit Logging in Feedback API]
**Vulnerability:** The public-facing `/api/feedback` endpoint lacked IP-based rate limiting or security audit logging of feedback submissions, exposing the application to cost exhaustion (spamming the LLM and ML classification pipeline) and a lack of accountability. Additionally, the admin-restricted approval route `/api/feedback/approve` lacked audit logging for sensitive decision-making actions (which trigger downstream Cursor-driven code deployment).
**Learning:** MVP components like the Feedback/Chat assistant and approval flow are often deployed with authentication but without rate-limiting thresholds or event logs, assuming access-control alone is sufficient. Unprotected unauthenticated/authenticated endpoints interacting with expensive LLM/ML systems are prime targets for DoS/abuse.
**Prevention:** Consistently apply `checkRateLimit` (with client IP extraction) and `logAuditEvent` to all public/authenticated endpoints that trigger generative AI or ML pipelines. Ensure all administrative approval or state-change routes log audit events to preserve non-repudiation.
