...
## 2026-07-12 - [Missing Rate Limiting and Audit Logging in DSAR Status Check]
**Vulnerability:** The `GET /api/governance/dsar/[token]` endpoint lacked IP-based rate limiting and security audit logging. This made it susceptible to brute-force token discovery and left no traceability for access to sensitive compliance request data.
**Learning:** Security protections implemented on mutation endpoints (like `POST /api/governance/dsar`) are not automatically inherited by related read-only or status-check endpoints. Token-based access endpoints often require even stricter perimeter defense as they bypass standard session-based authentication.
**Prevention:** Always apply `checkRateLimit` and `logAuditEvent` to both the creation and retrieval paths of token-authenticated resources. Ensure that "read" actions on sensitive data (PII/Compliance) are logged with the same rigor as "write" actions.
