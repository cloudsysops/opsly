...
## 2026-07-18 - [Missing Rate Limiting and Audit Logging on Compliance Verification Endpoint]
**Vulnerability:** The DSAR token verification GET endpoint `/api/governance/dsar/[token]` was exposed without any rate limiting or audit logging, despite returning highly sensitive compliance and personal data (subject email, request type, etc.) and executing state mutations (updating status to 'verified').
**Learning:** Security controls like rate limiting and auditing applied to resource creation (POST) should consistently be extended to associated status check or verification endpoints (GET) that interact with the same sensitive PII or compliance data, as they are otherwise vulnerable to brute-force token probes or unaccountable PII disclosure.
**Prevention:** Systematically apply `checkRateLimit` and `logAuditEvent` to all public endpoints handling verification or lookup of sensitive transactions, specifically within compliance/governance workflows. Verify these perimeters with dedicated security tests co-located in dynamic route folders.
