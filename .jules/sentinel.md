...
## 2026-07-06 - [Defense-in-depth for Administrative Endpoints]
**Vulnerability:** Administrative endpoints under `apps/api/app/api/defense/` lacked IP-based rate limiting and audit logging. While protected by session/token auth, they remained vulnerable to credential-stuffing (if session-based) or resource exhaustion by authorized but misbehaving/compromised administrative accounts.
**Learning:** Authentication wrappers (`requireAdminAccess`) do not implicitly provide perimeter defense or non-repudiation. Defense-in-depth requires explicit rate limiting and audit logging even for "trusted" administrative routes.
**Prevention:** Always apply `checkRateLimit` and `logAuditEvent` to administrative mutation endpoints. Utilize specific rate-limiting keys per endpoint to prevent cross-endpoint exhaustion.
