...
## 2026-07-09 - [Caching Web Dashboard Metrics]
**Learning:** The web dashboard summary endpoint was performing 11 parallel Supabase queries on every request. This is a significant bottleneck that scales poorly with database size and concurrent users. Implementing Redis caching (key: `metrics:web_dashboard_json`) with a short TTL (60s) reduces this O(N) database load to an O(1) cache lookup. Using the `void setCache(...).catch(...)` pattern ensures the cache write is non-blocking.
**Action:** Identify and aggregate multiple parallel database queries into a single cached JSON result for dashboard-facing API endpoints.
