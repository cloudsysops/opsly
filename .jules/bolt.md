...
## 2026-07-12 - [Caching Web Dashboard Metrics]
**Learning:** Dashboard endpoints that aggregate multiple parallel Supabase queries (e.g., 11 parallel calls for tenants, plans, and conversions) can put significant load on the database and increase network latency. Implementing Redis caching with a short TTL (60s) collapses these multiple network round-trips into a single O(1) cache lookup for the majority of requests. Using `void setCache(...)` ensures that cache writes don't block the API response.
**Action:** Identify dashboard aggregation functions that perform more than 5 parallel database queries and implement short-term Redis caching.
