...
## 2026-07-18 - [Supabase Query Builder Mocking in Parallel Operations]
**Learning:** When mocking complex Supabase/Postgrest builder chains in Vitest tests, do not return a single shared builder instance. Since builder objects are mutable and retain state (e.g., filter/query flags), using the same shared instance across multiple parallel queries (e.g. inside `Promise.all`) leaks query conditions across tests and queries.
**Action:** Always construct a nested mock client structure that instantiates a fresh chain/builder object on every distinct execution step to prevent test state leakage.
