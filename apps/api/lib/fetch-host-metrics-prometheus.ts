...
function toGb(bytes: number): number {
  return Number(
    (bytes / HOST_METRICS.BYTES_PER_GIB).toFixed(HOST_METRICS.DECIMALS_GB),
  );
}

function scalarFrom(
  res: PromVectorResult | null,
  mode: "avg" | "sum",
): number | null {
  if (res === null) {
    return null;
...
function bundleFromScalars(
  ...
  return {
    cpu_percent: Math.min(
      ...
      Math.max(
        HOST_METRICS.CPU_MIN_PCT,
        Number(cpuRaw.toFixed(HOST_METRICS.DECIMALS_CPU)),
      ),
    ),
