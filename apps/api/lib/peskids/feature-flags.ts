...
export function isPeskidsHotLeadAlertsEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return parseBooleanFlag(env.PESKIDS_HOT_LEAD_ALERTS_ENABLED, false);
}

export function isPeskidsDailyDigestEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
...
export function isPeskidsLeadConfirmationEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return parseBooleanFlag(env.PESKIDS_LEAD_CONFIRMATION_ENABLED, false);
}

export function isPeskidsLeadReminder24hEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return parseBooleanFlag(env.PESKIDS_LEAD_REMINDER_24H_ENABLED, false);
}

export function isPeskidsLeadEscalation48hEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return parseBooleanFlag(env.PESKIDS_LEAD_ESCALATION_48H_ENABLED, false);
}

export function isPeskidsAutoCreateFollowupEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return parseBooleanFlag(env.PESKIDS_AUTO_CREATE_FOLLOWUP_ENABLED, false);
}

export function isPeskidsTrialReminderEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
...
export function getPeskidsContactSlaHours(env: NodeJS.ProcessEnv = process.env): number {
