...
function parseFeedbackPostFields(
  ...
  if (rawEmail !== undefined && rawEmail.toLowerCase() !== identity.user_email.toLowerCase()) {
    return Response.json(
  ...
  if (!message) {
    return Response.json({ error: 'message es requerido' }, { status: HTTP_STATUS.BAD_REQUEST });
  }
