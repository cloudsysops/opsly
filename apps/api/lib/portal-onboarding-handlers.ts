...
const OnboardingBodySchema = z.object({
  org_name: z.string().min(LIMITS.ORG_NAME_MIN).max(LIMITS.ORG_NAME_MAX).trim(),
  slug: z
    .string()
    .regex(new RegExp(`^[a-z0-9-]{${String(LIMITS.SLUG_MIN)},${String(LIMITS.SLUG_MAX)}}$`)),
...
async function readValidatedOnboardingBody(request: Request): Promise<ValidatedOnboardingBody> {
...
function responseForTenantInsertFailure(tenantResult: { error: string; code?: string }): Response {
