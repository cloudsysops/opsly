 ...
 * Caches result in Redis for CACHE_TTL.SHORT (60s) to reduce DB load.
 */
export async function checkTenantBudget(tenantId: string): Promise<TenantBudgetCheckResult> {
  const cacheKey = `tenant:budget:check:${tenantId}`;
  ...
  // Cache for 60 seconds to avoid O(N) DB hits in dashboard loops
