...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  void logAuditEvent({ tenant_slug: parsed.data.tenant_id, action: 'CREATE', resource: `governance:breach:${data.id}`, ip, user_agent: request.headers.get('user-agent') ?? undefined, metadata: { title: parsed.data.title, severity: parsed.data.severity, discovered_at: parsed.data.discovered_at } });
