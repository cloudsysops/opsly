...
describe('POST /api/governance/consent security', () => {
  ...
  it('logs an audit event and records consent on successful request', async () => {
    vi.mocked(rateLimiter.checkRateLimit).mockResolvedValue({
      allowed: true,
      remaining: 99,
      resetAt: new Date(),
    ...
    const mockInsert = vi.fn().mockReturnValue({
      select: vi.fn().mockReturnValue({
        single: vi.fn().mockResolvedValue({
          data: { id: 'consent-456', granted_at: new Date().toISOString() },
          error: null,
        }),
      }),
    });
    ...
    expect(body.consent_id).toBe('consent-456');

    expect(audit.logAuditEvent).toHaveBeenCalledWith(
      expect.objectContaining({
        tenant_slug: 'test-tenant',
        action: 'governance_consent_record',
        resource: 'consent:consent-456',
      })
    );
  });
});
