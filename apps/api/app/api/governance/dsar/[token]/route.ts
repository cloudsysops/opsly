...
export async function GET(
  ...
  const rateLimit = await checkRateLimit(ip ? `governance:dsar-verify:${ip}` : 'governance:dsar-verify:anonymous');
  ...
  if (error || !data) {
    void logAuditEvent({
      ...
      action: 'VERIFY_FAIL',
      resource: `dsar-token:${token.slice(0, 8)}...`,
      ip,
      user_agent: request.headers.get('user-agent') ?? undefined,
      metadata: { error: error ? error.message : 'Not found' },
    });
