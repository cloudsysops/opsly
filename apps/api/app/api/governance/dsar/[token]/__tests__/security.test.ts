...
describe('GET /api/governance/dsar/[token] security', () => {
  ...
  it('returns 429 when rate limit is exceeded', async () => {
    ...
    const request = new NextRequest(
      'http://localhost/api/governance/dsar/longverificationtoken123',
      {
        method: 'GET',
      }
    );
    ...
    const response = await GET(request, {
      params: Promise.resolve({ token: 'longverificationtoken123' }),
    });
  ...
  it('logs an audit event on successful token verification', async () => {
    vi.mocked(rateLimiter.checkRateLimit).mockResolvedValue({
      allowed: true,
      remaining: 99,
      resetAt: new Date(),
