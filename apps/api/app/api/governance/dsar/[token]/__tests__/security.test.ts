...
describe('GET /api/governance/dsar/[token] security', () => {
  ...
  it('returns 429 when rate limit is exceeded', async () => {
    ...
    const response = await GET(request, {
      params: Promise.resolve({ token: 'token1234567890' }),
    });
  ...
  it('logs an audit event when token is successfully verified', async () => {
    vi.mocked(rateLimiter.checkRateLimit).mockResolvedValue({
      allowed: true,
      remaining: 99,
      resetAt: new Date(),
    ...
    const request = new NextRequest('http://localhost/api/governance/dsar/validtoken12345', {
      method: 'GET',
    });

    const response = await GET(request, {
      params: Promise.resolve({ token: 'validtoken12345' }),
    });
