...
describe("PUT /api/portal/tenant/[slug]/budget", () => {
  ...
  it("returns 401 when session fails", async () => {
    ...
    const res = await PUT(
      makeReq("PUT", { monthly_cap_usd: 100, alert_threshold_pct: 80 }),
      { params: Promise.resolve({ slug: SLUG }) },
    );
    expect(res.status).toBe(401);
  });
  ...
  it("returns 400 for alert_threshold_pct out of range", async () => {
    okSession();
    const res = await PUT(
      makeReq("PUT", { monthly_cap_usd: 50, alert_threshold_pct: 110 }),
      { params: Promise.resolve({ slug: SLUG }) },
    );
    expect(res.status).toBe(400);
  });
  ...
  it("returns 200 on valid PUT", async () => {
    ...
    mockDb.mockReturnValue(makeSelectDb(null) as never);
    const res = await PUT(
      makeReq("PUT", { monthly_cap_usd: 99.5, alert_threshold_pct: 75 }),
      { params: Promise.resolve({ slug: SLUG }) },
    );
    expect(res.status).toBe(200);
    const body = await res.json();
