...
export async function POST(request: Request): Promise<Response> {
  ...
  const body = (await request.json().catch(() => null)) as { tool?: unknown; actor?: unknown } | null;
  const toolId = typeof body?.tool === 'string' ? body.tool.trim() : '';
  if (!toolId) {
    return Response.json({ error: 'tool is required' }, { status: 400 });
  }

  const plan = await createInstallPlan(toolId, typeof body?.actor === 'string' ? body.actor : 'admin');
  if (!plan) {
