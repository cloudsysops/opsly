  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-pk-bg px-4">
      <div className="w-full max-w-md rounded-2xl border border-pk-border bg-white p-8 shadow-card">
        ...
        <p className="text-center text-[10px] font-semibold uppercase tracking-[0.18em] text-pk-mutedText">
          {surface === 'support' ? 'Soporte' : surface === 'teacher' ? 'Profesores' : 'Admin'}
        </p>
        <h1 className="mt-2 font-display text-center text-xl font-semibold text-pk-ink">
          {title}
        </h1>
        <p className="mt-2 text-center text-sm text-pk-sub">
          {ready ? description : 'Abre el enlace del correo de recuperación o solicita uno nuevo en login.'}
        </p>
...
export function UpdatePasswordForm(props: UpdatePasswordFormProps): React.ReactElement {
  return (
    <Suspense
      fallback={
