 ...
 */
export function calculateTeamScores(stickersByCountry: Map<string, number>): TeamScore[] {
  ...
  const scores = TEAMS.map((team) => {
    ...
    const powerScore = Math.round(
      rankScore * 0.4 + formScore * 0.35 + expScore * 0.15 + collectionBonus * 0.5
    );
...
export function headToHead(
  ...
  const label = prob >= 70 ? 'Favorito claro' : prob >= 58 ? 'Ligera ventaja' : 'Parejo';
