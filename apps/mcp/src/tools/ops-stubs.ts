...
function buildRestartPrompt(container: string): string {
  return [
    '# restart_container — solicitud operativa (OpenClaw MCP)',
    `# container: ${container}`,
    '',
    '## Acción requerida',
    `Reiniciar el contenedor Docker \`${container}\` en el VPS siguiendo el runbook de operaciones.`,
    'Verificar en Mission Control / docker ps que el nombre coincide antes de reiniciar.',
    'No ejecutar comandos shell desde este archivo; usar SSH + runbook aprobado.',
  ].join('\n');
}
