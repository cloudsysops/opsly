...
describe("OpenClawMcpServer OAuth scopes", () => {
  ...
  it("allows tool when Bearer token has required scope", async () => {
    const token = generateAccessToken("claude-ai", ["tenants:read", "tenants:write"], undefined, 600);
  ...
  it("rejects when Bearer token lacks scope", async () => {
    ...
    const server = createServer();
    await expect(
      server.callTool("onboard_tenant", {
        slug: "abc-slug-test",
        email: "a@b.com",
        plan: "startup",
  ...
  it("accepts PLATFORM_ADMIN_TOKEN as Bearer for any tool", async () => {
    const server = createServer();
    const out = await server.callTool("get_tenants", {}, {
