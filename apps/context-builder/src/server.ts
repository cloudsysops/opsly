...
function authorizeContextPack(req: import('node:http').IncomingMessage): boolean {
  const token = process.env.CONTEXT_PACK_TOKEN?.trim() || process.env.PLATFORM_ADMIN_TOKEN?.trim();
  if (!token) {
    return false;
  }
