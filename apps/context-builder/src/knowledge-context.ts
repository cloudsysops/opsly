...
import { CONTEXT_CACHE_TTL_SECONDS, MAX_CONTEXT_CHARS, MAX_CONTEXT_FILES } from './constants.js';
...
export async function buildContextFromQuery(query: string): Promise<BuildContextResult> {
  ...
  if (!index) {
    const empty =
      '<context_bundle><!-- knowledge-index.json missing or invalid --></context_bundle>';
  ...
  for (const rel of paths) {
    ...
    const piece =
      inner.length > room ? `${inner.slice(0, room)}\n<!-- truncated -->\n${close}` : inner;
