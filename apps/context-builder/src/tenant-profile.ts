...
export function resolveTenantIdentity(
  ...
  const tech_stack =
    fromColumnStack && Object.keys(fromColumnStack).length > 0
      ? fromColumnStack
      : fromMetaStack;

  const csCol = row.coding_standards?.trim();
  const csMeta =
    typeof metadata.coding_standards === "string"
      ? metadata.coding_standards.trim()
      : "";
  const coding_standards =
    csCol && csCol.length > 0
      ? csCol
      : csMeta.length > 0
        ? csMeta
        : undefined;
  ...
  const business_domain =
    typeof metadata.business_domain === "string"
...
export function buildIdentityPromptBlock(
  name: string,
  identity: TenantIdentityResolved,
): string {
  const lines: string[] = [
