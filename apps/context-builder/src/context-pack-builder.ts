...
import { ContextPackSchema, type ContextPack, type ContextPackAdr } from '@intcloudsysops/types';
import { loadKnowledgeIndex, repoRoot } from './knowledge-index.js';
import { buildIdentityPromptBlock, resolveTenantIdentity } from './tenant-profile.js';
...
function parseAdrFile(filename: string, content: string): ContextPackAdr {
  ...
  if (!summary) {
    summary = content.slice(0, 400).replace(/\s+/g, ' ').trim();
  }
...
export async function buildContextPack(input: BuildContextPackInput): Promise<ContextPack> {
  ...
  if (input.tenantId?.trim()) {
    const { data, error } = await base.eq('id', input.tenantId.trim()).maybeSingle();
    if (error) {
      warnings.push(`tenant_query:${error.message}`);
    }
    tenantRow = data;
  } else if (input.tenantSlug?.trim()) {
    const { data, error } = await base.eq('slug', input.tenantSlug.trim()).maybeSingle();
    if (error) {
      warnings.push(`tenant_query:${error.message}`);
    }
    tenantRow = data;
  ...
  const { tech_stack, coding_standards, vector_namespace, domain, business_domain } = resolved;
