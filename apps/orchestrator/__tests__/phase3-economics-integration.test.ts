...
describe('Phase 3 Economics - Integration Tests', () => {
  ...
  describe('Optimization 3: Polling Reduction (20% → 12%)', () => {
    it('polling interval: 3s vs 1s reduces Redis ops by 66%', () => {
      const intervalOptimizedMs = 3000; // 3 second poll
      const intervalLegacyMs = 1000; // 1 second poll
      const pollsPerSecondLegacy = 1000 / intervalLegacyMs;
      const pollsPerSecondOptimized = 1000 / intervalOptimizedMs;
      const reductionPercent =
        ((pollsPerSecondLegacy - pollsPerSecondOptimized) / pollsPerSecondLegacy) * 100;
  ...
  describe('Backward Compatibility & Rollback', () => {
    ...
    it('polling: env var ORCHESTRATOR_POLLING_OPTIMIZED disables optimization', () => {
      ...
      // Should use LEGACY_DRAIN_PROFILE (1s drain delay)
