...
export async function enqueueValidationJob(job: OrchestratorJob) {
  if (job.type !== 'test_validation') {
    throw new Error(`enqueueValidationJob: unsupported type ${job.type}`);
  }
  const opts = buildQueueAddOptions(job);
  const bull = await validationQueue.add(job.type, job, opts);
  ...
  logJobEnqueue({
    ...
    autonomy_risk: job.autonomy_risk,
    queue_priority: opts.priority,
    metadata: { ...(job.metadata ?? {}), queue: 'validation' },
  });
