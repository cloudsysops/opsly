...
async function handleValidationPass(
  ...
  const { task_id, agent, commit_sha, pr_url } = job.data;
...
async function handleValidationFail(
  job: Job<ValidationPayload>,
  conclusion: CIConclusion,
  t0: number
) {
  const { task_id, commit_sha, pr_url, retry_count = 0 } = job.data;
...
async function handleValidationJob(job: Job<ValidationPayload>, t0: number) {
  if (job.name !== 'validate') return;

  const { commit_sha } = job.data;
  ...
  if (conclusion === 'success') {
    return handleValidationPass(job, conclusion, t0);
  }

  return handleValidationFail(job, conclusion, t0);
}
...
export function startValidationWorker(connection: object): Worker {
  return new Worker<ValidationPayload>(
    ...
    async (job: Job<ValidationPayload>) => {
      ...
      return handleValidationJob(job, t0);
    },
