...
export function startSuperOrchestratorWorker(): Worker {
  const worker = new Worker<SuperOrchestratorJobData>(
    ...
    async (job: Job<SuperOrchestratorJobData>) => {
      ...
      console.log(
        `[super-orchestrator-worker] Processing job ${job.id}: ${prompt.substring(0, 50)}...`
      );
  ...
  console.log(`[super-orchestrator-worker] Started with concurrency ${WORKER_CONCURRENCY}`);
