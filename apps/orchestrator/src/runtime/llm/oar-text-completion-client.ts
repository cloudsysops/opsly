...
export function createOarTextCompletionClient(
  ...
  return {
    async complete(_model: string, prompt: string): Promise<string> {
      ...
      if (!res.ok) {
        throw new Error(`OAR llm-gateway /v1/text HTTP ${String(res.status)}: ${text.slice(0, 400)}`);
      }
      ...
      if (meter) {
        const llm = (parsed as { llm?: { tokens_input?: unknown; tokens_output?: unknown; model_used?: unknown } })
          .llm;
