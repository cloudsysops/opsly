...
Peskids is **the first real tenant** incubated within the Opsly monorepo. It is NOT yet a standalone product repo (see [EXTRACTION-PLAN.md](docs/tenants/peskids/EXTRACTION-PLAN.md) for future extraction criteria).

**Current setup (Phase 0):**
- App lives at `apps/peskids/` (Next.js 14)
- Schema lives in shared Supabase project (`jkwykpldnitavhmtuzmo`)
- n8n CRM workflows run in VPS container `tenant_peskids`
- Metadata in `config/tenants/peskids.json`
- Docs in `docs/tenants/peskids/`

**Future extraction (Phase 1+):**
- Will move to standalone repo `cloudsysops/peskids-platform`
...
**Core entities** (see `docs/tenants/peskids/DATA-MODEL.md` for full schema):

| Entity | Purpose | Key fields |
|--------|---------|-----------|
| `leads` | Prospect intake | source, status, full_name, email, phone |
| `parents` | Guardians enrolled | email, phone, preferred_channel |
| `students` | Children in program | parent_id, grade_or_level, status |
| `teachers` | Staff | email, subjects, active |
| `classes` | Program offerings | name, teacher_id, schedule, enrollments |
| `feedback` | Parent/teacher input | author_type, subject_type, rating, status |
| `followups` | Action items | related_type, due_at, assigned_to, status |
| `messages` | Approval-gated comms (future) | channel, direction, approval_status |
| `content_ideas` | Draft content (future) | publish_status |

All tables have `tenant_slug = 'peskids'` hardcoded during incubation (will parameterize during extraction).

### Role-based access (future RLS)

| Role | Access |
|------|--------|
| **owner** (sierrasantiago90@gmail.com) | All data, tenant admin |
| **staff** | Leads, students, classes, feedback |
| **parent** | Own children, own feedback only |
| **teacher** | Own classes, feedback on own classes |
