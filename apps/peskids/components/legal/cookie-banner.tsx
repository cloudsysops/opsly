...
/**
 * Marketing cookies (Meta Pixel) only exist when a Pixel ID is actually
 * configured. Keeping the banner's copy conditional on that means it never
 * claims marketing cookies exist before they do, or the reverse.
 */
...
export function CookieBanner(): React.ReactElement | null {
  ...
  const acceptAll = (): void => {
    writeCookieConsent(HAS_MARKETING_PIXEL);
    setVisible(false);
  };
  ...
  return (
    ...
    >
      <div className="flex items-start gap-2.5 sm:gap-3">
        <div className="flex-1">
          <p className="text-[13px] font-semibold text-pk-ink sm:text-sm">
            {HAS_MARKETING_PIXEL ? 'Usamos cookies' : 'Usamos cookies esenciales'}
          </p>
          <p className="mt-0.5 text-[10px] leading-snug text-pk-sub sm:mt-1 sm:text-[11px] sm:leading-relaxed">
            {HAS_MARKETING_PIXEL ? (
              <>
                ...
                staff) y, si lo aceptas, cookies de Meta (Instagram/Facebook) para medir la
      ...
      <div className="mt-2 flex flex-wrap gap-1.5 sm:mt-3 sm:gap-2">
        ...
        >
          {HAS_MARKETING_PIXEL ? 'Aceptar todo' : 'Entendido'}
        </button>
        {HAS_MARKETING_PIXEL ? (
          <button
            type="button"
