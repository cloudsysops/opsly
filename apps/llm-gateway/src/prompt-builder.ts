...
export function buildPrompt(userRequest: string, intent: DetectedIntent, ctx: EnrichedContext): string {
  const tech = [
    `Stack: Next.js 15, TypeScript estricto (sin any), Tailwind, Supabase, Docker Compose, Traefik v3.`,
    intent.affected_area ? `Área detectada: ${intent.affected_area} (equipo sugerido: ${intent.suggested_team}).` : "",
