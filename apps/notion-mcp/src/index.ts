...
app.post("/mcp/tasks/create", async (req, res) => {
  try {
    const result = await handleCreateTask(
      req.body as Parameters<typeof handleCreateTask>[0],
    );
    res.json(result);
  } catch (err: unknown) {
    res.status(400).json({ error: errMessage(err) });
...
app.post("/mcp/standup/add", async (req, res) => {
  try {
    const result = await handleAddStandup(
      req.body as Parameters<typeof handleAddStandup>[0],
    );
    res.json(result);
  } catch (err: unknown) {
    res.status(400).json({ error: errMessage(err) });
...
app.post("/mcp/quality-gate/record", async (req, res) => {
  try {
    const result = await handleQualityGate(
      req.body as Parameters<typeof handleQualityGate>[0],
    );
    res.json(result);
  } catch (err: unknown) {
    res.status(400).json({ error: errMessage(err) });
