...
export default function AdminAgentsPage() {
  const { data, error, isLoading } = useSWR(['teams-metrics'], () => getTeamMetrics(), {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <h1 className="font-mono text-lg tracking-tight text-ops-green">Agent Teams</h1>
  ...
  return (
    <div className="rounded border border-ops-border/60 p-4">
      ...
      <div className="mt-3 font-mono text-[11px] text-ops-gray">
        queue/active: {team.waiting !== undefined ? `${team.waiting} / ${team.active ?? 0}` : '—'}
      </div>
    </div>
  );
}
