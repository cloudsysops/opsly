    ...
    <div className="mx-auto max-w-5xl p-8">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-400">
            Datos desde <code className="text-neutral-300">mac2011-monitor.sh</code> vía{' '}
            <code className="text-neutral-300">MAC2011_STATUS_URL</code> o{' '}
            <code className="text-neutral-300">MAC2011_STATUS_FILE</code> en la API.
          </p>
          {data ? (
      ...
      {notConfiguredHint && !data ? (
        <div className="mb-6 rounded border border-amber-900/50 bg-amber-950/20 px-4 py-3 text-sm text-amber-100">
          <p className="font-mono font-semibold text-amber-300">API sin fuente de métricas</p>
      ...
      {data ? (
        <>
          <div className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-4">
            ...
            />
            <ResourceCard title="Docker" value={data.docker.containers} unit=" cont." tone="blue" />
          </div>

          <div className="mb-8 rounded border border-ops-border bg-ops-surface p-6">
            <h2 className="mb-4 font-mono text-sm font-semibold text-neutral-200">Workers</h2>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <div className="flex items-center justify-between rounded border border-ops-border px-4 py-3">
                ...
                {statusBadge(data.workers.ollama === 'running', data.workers.ollama)}
              </div>
              <div className="flex items-center justify-between rounded border border-ops-border px-4 py-3">
                <span className="text-neutral-300">Worker Opsly</span>
                {statusBadge(data.workers.opsly_worker === 'running', data.workers.opsly_worker)}
              </div>
            </div>
          </div>

          <div className="rounded border border-ops-border bg-ops-surface p-6">
            <h2 className="mb-4 font-mono text-sm font-semibold text-neutral-200">Red</h2>
            <div className="flex items-center justify-between rounded border border-ops-border px-4 py-3">
              <span className="text-neutral-300">VPS (ping Tailscale)</span>
              {statusBadge(data.network.vps_connection === 'up', data.network.vps_connection)}
            </div>
          </div>
        </>
      ) : null}
