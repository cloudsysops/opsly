      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-ops-muted">
            {alerts.length} tenant{alerts.length === 1 ? "" : "s"} requieren
            revisión por consumo o proyección.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          {criticalCount > 0 ? (
            <Badge variant="red">Críticos: {criticalCount}</Badge>
          ) : null}
          {warningCount > 0 ? (
            <Badge variant="yellow">Avisos: {warningCount}</Badge>
          ) : null}
        </div>
      </div>
        ...
        {alerts.map((s) => (
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {s.enforcement_skipped ? (
                <Badge variant="gray">Enforcement omitido</Badge>
              ) : null}
              {s.projected_month_end_usd > s.limit_usd ? (
                <Badge variant="blue">
                  Proyección: ${s.projected_month_end_usd.toFixed(2)}
                </Badge>
              ) : null}
            </div>
