...
test('a failed runtime session remains visible as ERROR', () => {
  const projection = buildMissionControlExecutionProjectionV1({
    runtime_sessions: [
      {
        ...
        agent_id: 'opencode',
        status: 'failed',
        branch: 'agent/opencode/work-failed',
  ...
  assert.equal(projection.activities.length, 1);
  assert.equal(projection.activities[0]?.state, 'ERROR');
  assert.equal(projection.activities[0]?.work_id, 'work-failed');
  assert.match(projection.activities[0]?.blocker ?? '', /reported failure/);
});
