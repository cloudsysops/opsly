...
test.describe("Portal — public pages", () => {
  ...
  test("login page renders heading, email and password inputs", async ({
    page,
  }) => {
    const errors: string[] = [];
...
test.describe("Portal — auth-protected pages", () => {
  ...
  test("dashboard/developer redirects to /login when unauthenticated", async ({
    page,
  }) => {
  ...
  test("dashboard/managed redirects to /login when unauthenticated", async ({
    page,
  }) => {
