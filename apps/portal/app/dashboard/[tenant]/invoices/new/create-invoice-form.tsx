...
export function CreateInvoiceForm({ tenant }: { tenant: string }) {
  ...
  const inputClass =
    'w-full rounded-sm border border-ops-border bg-ops-bg px-3 py-2 text-sm text-neutral-100 placeholder:text-ops-gray outline-none transition-all focus-visible:border-ops-green focus-visible:ring-2 focus-visible:ring-ops-green/30 disabled:cursor-not-allowed disabled:opacity-50';
    ...
    <form onSubmit={(e) => void handleSubmit(e)} className="space-y-6">
      ...
      <div className="flex justify-end">
        <Button type="submit" variant="primary" disabled={loading}>
          {loading ? (
            <>
              <Loader2 className="h-4 w-4 shrink-0 animate-spin mr-1.5" aria-hidden="true" />
              Creando...
            </>
          ) : (
            'Crear Factura'
