...
export function InsightDashboard({ tenantSlug, insights: initial }: Props): ReactElement {
  ...
  const patch = useCallback(
    async (insightId: string, action: 'read' | 'dismiss' | 'action') => {
      ...
      if (action === 'read') {
        setAnnouncement('Marcando insight como leído...');
      } else if (action === 'dismiss') {
        setAnnouncement('Descartando insight...');
      } else {
        setAnnouncement('Procesando acción...');
      }
      try {
        ...
        if (!session?.access_token) {
          setAnnouncement('Error: Sesión no válida');
          return;
        }
        ...
        if (action === 'dismiss' || action === 'action') {
          ...
          setAnnouncement(action === 'dismiss' ? 'Insight descartado' : 'Acción completada');
        } else {
          setInsights((prev) =>
