...
# Copies 0001-0097 by default. 0100 is opt-in for the Peskids operations RLS
# validation only; franchise migrations 0098/0099 remain excluded.
#
# Usage:
#   ./scripts/peskids-apply-staging-schema.sh --dry-run
#   ./scripts/peskids-apply-staging-schema.sh --dry-run --include-operations-rls
...
TARGET_REF="$QA_REF"
DRY_RUN=false
INCLUDE_OPERATIONS_RLS=false

while [[ $# -gt 0 ]]; do
  case "$1" in
    --dry-run) DRY_RUN=true; shift ;;
    --include-operations-rls) INCLUDE_OPERATIONS_RLS=true; shift ;;
...
if [[ "$INCLUDE_OPERATIONS_RLS" == "true" && "$TARGET_REF" != "$QA_REF" ]]; then
  echo "Refusing 0100 apply: operations RLS validation is allowed only on the configured QA project (${QA_REF})." >&2
  exit 1
fi
...
if [[ "$INCLUDE_OPERATIONS_RLS" == "true" ]]; then
  operations_rls="$ROOT/supabase/migrations/0100_peskids_operations_tenant_rls.sql"
  [[ -f "$operations_rls" ]] || {
    echo "Refusing: missing Peskids operations RLS migration: ${operations_rls}" >&2
    exit 1
  }
  cp "$operations_rls" "$WORKDIR/supabase/migrations/"
fi
...
if [[ "$INCLUDE_OPERATIONS_RLS" == "true" ]]; then
  echo "[peskids-schema] copied=${copied} plus=0100_peskids_operations_tenant_rls max=0100 excluded=0098/0099"
else
...
if [[ "$DRY_RUN" == "true" ]]; then
  if [[ "$INCLUDE_OPERATIONS_RLS" == "true" ]]; then
    echo "[peskids-schema] DRY-RUN: would link ${TARGET_REF} and db push 0001-0097 plus 0100 operations RLS"
  else
...
if [[ "$INCLUDE_OPERATIONS_RLS" == "true" ]]; then
  echo "[peskids-schema] applied 0001-0097 plus 0100 operations RLS on ${TARGET_REF}"
else
