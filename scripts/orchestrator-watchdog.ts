...
class OrchestratorWatchdog {
  ...
  async run(): Promise<void> {
    ...
    if (lastCheck.status !== 'down') {
      await this.checkAndRecoverSessions();
    }
  ...
  private async checkAndRecoverSessions(): Promise<void> {
    const recoveryEndpoint =
      process.env.RECOVERY_ENDPOINT || `http://localhost:${this.port}/internal/sessions/recover`;

    try {
      const response = await this.makeHttpRequest(recoveryEndpoint, 10000);
      if (response.statusCode === 200) {
        this.log('Session recovery check: OK', 'info');
      } else {
        this.log(`Session recovery check returned: ${response.statusCode}`, 'warn');
      }
    } catch (err) {
      this.log(
        `Session recovery check failed: ${err instanceof Error ? err.message : String(err)}`,
        'warn'
      );
    }
  }
