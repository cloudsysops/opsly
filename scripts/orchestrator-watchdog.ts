...
class OrchestratorWatchdog {
  ...
  private backoffMs = [1000, 2000, 4000, 8000]; // exponential backoff
  ...
  private async restartOrchestrator(): Promise<boolean> {
    ...
    const waitMs = this.backoffMs[backoffIndex];

    this.log(`Waiting ${waitMs}ms before restart attempt ${this.state.restartCount + 1}/${this.maxRetries}...`);
    await new Promise((resolve) => setTimeout(resolve, waitMs));

    try {
      this.log('Attempting docker-compose restart...');
      const cmd = 'docker compose -f infra/docker-compose.openclaw-orchestrator.yml restart orchestrator';
      execSync(cmd, { stdio: 'inherit', cwd: '/home/user/opsly' });
  ...
  private notifyEscalation(result: HealthCheckResult): void {
    ...
    try {
      const cmd = `node /home/user/opsly/scripts/notify-discord-orchestrator.js escalation '${JSON.stringify(result)}'`;
