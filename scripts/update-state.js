...
const apps = fs.readdirSync(path.join(ROOT, 'apps')).filter((d) =>
  fs.statSync(path.join(ROOT, 'apps', d)).isDirectory()
)
...
const migrations = fs.existsSync(path.join(ROOT, 'supabase/migrations'))
  ? fs.readdirSync(path.join(ROOT, 'supabase/migrations')).filter((f) =>
      f.endsWith('.sql')
    )
  : []
