...
HEALTH_V1=$(curl -sf --max-time 10 "${API_URL}/api/v1/health" 2>/dev/null || echo '{"status":"error"}')
check "Health v1 Prefix" "$(echo "$HEALTH_V1" | grep -q '"status":"ok"' && echo OK || echo FAIL)"

# Test 3: API Docs (Swagger)
DOCS=$(curl -sf --max-time 10 -o /dev/null -w "%{http_code}" "${API_URL}/api/docs" 2>/dev/null || echo "000")
check "API Docs (Swagger)" "$([ "$DOCS" = "200" ] && echo OK || echo FAIL)"

# Test 4: OpenAPI YAML
OPENAPI=$(curl -sf --max-time 10 -o /dev/null -w "%{http_code}" "${API_URL}/openapi.yaml" 2>/dev/null || echo "000")
check "OpenAPI Spec" "$([ "$OPENAPI" = "200" ] && echo OK || echo FAIL)"

# Test 5: API v1 Info
APIV1=$(curl -sf --max-time 10 -o /dev/null -w "%{http_code}" "${API_URL}/api/v1" 2>/dev/null || echo "000")
check "API v1 Info" "$([ "$APIV1" = "200" ] && echo OK || echo FAIL)"

# Test 6: Portal Health (requiere ?slug=)
PORTAL=$(curl -sf --max-time 10 "${API_URL}/api/portal/health?slug=${PORTAL_HEALTH_SLUG}" 2>/dev/null || echo '{"status":"error"}')
...
SPRINTS=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" "${API_URL}/api/sprints/active" 2>/dev/null || echo "000")
...
if [ -n "$ADMIN_TOKEN" ]; then
  ...
  METRICS=$(curl -sf --max-time 10 -H "Authorization: Bearer $ADMIN_TOKEN" -o /dev/null -w "%{http_code}" "${API_URL}/api/metrics" 2>/dev/null || echo "000")
  check "Admin Metrics" "$([ "$METRICS" = "200" ] && echo OK || echo FAIL)"

  TENANTS=$(curl -sf --max-time 10 -H "Authorization: Bearer $ADMIN_TOKEN" -o /dev/null -w "%{http_code}" "${API_URL}/api/tenants" 2>/dev/null || echo "000")
