...
export function createCreatorLocalBridge(options = {}) {
  ...
  const server = createServer((req, res) => {
    ...
    if (method === 'GET' && requestUrl.pathname === '/health') {
      sendJson(res, 200, { status: 'ok', binding: 'loopback', sessionId });
      return;
    }
  ...
  return {
    ...
    async start() {
      if (server.listening) throw new Error('Creator Local Bridge is already listening');
...
if (invokedPath && fileURLToPath(import.meta.url) === invokedPath) {
  runCli().catch(error => {
    process.stderr.write(
      `CREATOR_LOCAL_BRIDGE_ERROR ${error instanceof Error ? error.message : String(error)}\n`,
    );
    process.exit(1);
  });
}
