...
export function evaluateCloudCostPolicy(input = {}, policy = {}) {
  const blockers = [];
  const warnings = [];
  ...
  for (const pattern of input.architecture_patterns || []) {
    if ((policy.prohibitedPatterns || []).includes(pattern)) {
      blockers.push(`PROHIBITED_ARCHITECTURE:${pattern}`);
    }
  }
...
if (isCliInvocation()) {
  ...
  const result = evaluateCloudCostPolicy(input, policy);
