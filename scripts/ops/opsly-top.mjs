...
export async function collectOpslyTopSnapshot(options = {}) {
  ...
  const endpoints = {
    ...
    dockerContainers: '/api/admin/docker/containers',
    dockerResources: '/api/admin/docker/resources',
  ...
  const dockerContainers = sourceResults.dockerContainers.data || {};
  const dockerResources = sourceResults.dockerResources.data || {};
  const overview = sourceResults.overview.data || {};
  ...
  for (const worker of compute.workers || []) {
    machines.push({
      ...
      docker_containers: worker.dockerContainers ?? null,
  ...
  const dockerResourceByName = new Map(
    (dockerResources.containers || []).map((row) => [String(row.name || '').replace(/^\//, ''), row]),
  );
  const containers = (dockerContainers.containers || []).map((container) => {
    const name = container.names?.[0] || container.id;
    const resource = dockerResourceByName.get(name) || null;
    return {
      id: container.id,
      name,
      ...
      cpu_pct: resource?.cpu_percent ?? null,
      memory_usage: resource?.memory_usage || null,
      memory_pct: resource?.memory_percent ?? null,
      net_io: resource?.net_io || null,
      block_io: resource?.block_io || null,
      pids: resource?.pids ?? null,
    };
  });
  ...
  return {
    ...
    summary: {
      ...
      containers_running: containers.filter((row) => row.state === 'running').length,
      vps_cpu_pct:
        overview?.vps_host?.mock !== true && typeof overview?.vps_host?.cpu_percent === 'number'
          ? overview.vps_host.cpu_percent
          : null,
      vps_ram_used_gb:
        overview?.vps_host?.mock !== true && typeof overview?.vps_host?.ram_used_gb === 'number'
          ? overview.vps_host.ram_used_gb
          : null,
      vps_ram_total_gb:
        overview?.vps_host?.mock !== true && typeof overview?.vps_host?.ram_total_gb === 'number'
          ? overview.vps_host.ram_total_gb
          : null,
    },
    machines,
    agents,
    queues,
    containers,
    host: overview?.vps_host?.mock === true ? null : overview?.vps_host || null,
...
export function renderOpslyTop(snapshot, options = {}) {
  ...
  lines.push(
    `machines ${snapshot.summary.machines_online}/${snapshot.summary.machines}  agents live ${snapshot.summary.agents_live}/${snapshot.summary.agents}  containers ${snapshot.summary.containers_running}/${snapshot.summary.containers_total}  sessions ${snapshot.summary.sessions_active}  claims ${snapshot.summary.claims_active}  queues W/A/F ${snapshot.summary.queue_waiting}/${snapshot.summary.queue_active}/${snapshot.summary.queue_failed}`,
  );
  lines.push('');
  lines.push(
    `${c.bold}HOST${c.reset}  CPU ${pct(snapshot.summary.vps_cpu_pct)}  RAM ${gb(snapshot.summary.vps_ram_used_gb)}/${gb(snapshot.summary.vps_ram_total_gb)}`,
  );
  lines.push('');

  lines.push(`${c.bold}MACHINES${c.reset}`);
  lines.push('HOST               STATE       CPU      RAM            GPU      VRAM           TEMP   JOBS DOCKER AGE');
  for (const m of snapshot.machines) {
    ...
    lines.push(
      [
        ...
        fit(number(m.docker_containers), 6),
  ...
  lines.push('');
  lines.push(`${c.bold}DOCKER CONTAINERS${c.reset}`);
  lines.push('CONTAINER                  STATE      CPU      MEMORY                    PIDS');
  for (const row of snapshot.containers.slice(0, 20)) {
    lines.push(
      [
        fit(row.name, 26),
        fit(row.state, 10),
        fit(pct(row.cpu_pct), 8),
        fit(
          row.memory_usage
            ? `${row.memory_usage}${row.memory_pct == null ? '' : ` · ${pct(row.memory_pct)}`}`
            : 'UNKNOWN',
          25,
        ),
        fit(number(row.pids), 5),
      ].join(' '),
    );
  }
  if (!snapshot.containers.length) lines.push('  no Docker evidence');
