...
async function main() {
  ...
  if (!notebookId || notebookId.length === 0) {
    process.stderr.write('NOTEBOOKLM_NOTEBOOK_ID no configurado. Skip.\n');
    process.exit(0);
  }

  let skills = [];
  try {
    skills = readdirSync(SKILLS_DIR).filter(
      (d) => d.startsWith('opsly-') && !d.startsWith('opsly-simplify')
    );
  } catch {
  ...
  for (const skillDir of skills) {
    ...
    try {
      ...
      if (result.success) {
        ...
        failed++;
        process.stderr.write(`  ❌ ${skillDir}: ${result.error ?? 'unknown error'}\n`);
      }
    } catch (e) {
      failed++;
      process.stderr.write(`  ❌ ${skillDir}: ${e instanceof Error ? e.message : String(e)}\n`);
    }
  }
