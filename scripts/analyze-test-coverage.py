...
@dataclass
class CoverageStats:
    """Test-file-to-source ratio statistics for a module (NOT code coverage %)."""
    name: str
    test_files: int
    source_files: int
    test_file_ratio: float
    status: str  # "critical", "high", "low", "very_low", "none"
    ...
    def coverage_str(self) -> str:
        ...
        return f"{self.test_file_ratio:.1f}%"


class CoverageAnalyzer:
    """Analyze test coverage across codebase."""

    CRITICAL_THRESHOLD = 70  # 70%+ test-file ratio
    HIGH_THRESHOLD = 50      # 50%+ test-file ratio
    LOW_THRESHOLD = 20       # 20%+ test-file ratio
    ...
    def _analyze_module(self, module_path: Path, prefix: str) -> None:
        ...
        # Calculate test-file-to-source ratio
        test_count = len(test_files)
        source_count = len(source_files)

        if source_count == 0:
            test_file_ratio = 0.0 if test_count == 0 else 100.0
        else:
            test_file_ratio = (test_count / source_count) * 100

        # Determine status
        if test_count == 0:
            status = "none"
        elif test_file_ratio >= self.CRITICAL_THRESHOLD:
            status = "critical"
        elif test_file_ratio >= self.HIGH_THRESHOLD:
            status = "high"
        elif test_file_ratio >= self.LOW_THRESHOLD:
            status = "low"
        else:
            status = "very_low"
    ...
    def get_summary(self) -> Dict:
        ...
        by_status = {"critical": [], "high": [], "low": [], "very_low": [], "none": []}
        total_tests = 0
        total_sources = 0
        ...
        return {
            ...
            "coverage_distribution": {
                ...
                "very_low": len(by_status["very_low"]),
    ...
    def print_report(self) -> None:
        ...
        # Print by status
        status_order = ["critical", "high", "low", "very_low", "none"]
        status_icons = {
            "critical": "🟢",
            "high": "🟡",
            "low": "🔴",
        ...
        for status in status_order:
            ...
            for stats in sorted(items, key=lambda s: s.test_file_ratio, reverse=True):
                print(
                    f"  {stats.name:<40} "
                    f"Tests: {stats.test_files:>3}  "
                    f"Sources: {stats.source_files:>3}  "
                    f"Ratio: {stats.coverage_str:>10}"
                )
        ...
        print(f"\nCritical (>=70% ratio):  {summary['coverage_distribution']['critical']}")
        print(f"High (50-70% ratio):     {summary['coverage_distribution']['high']}")
        print(f"Low (20-50% ratio):      {summary['coverage_distribution']['low']}")
        print(f"Very Low (<20% ratio):   {summary['coverage_distribution']['very_low']}")
        print(f"None (0% ratio):         {summary['coverage_distribution']['none']}")
        print("=" * 80 + "\n")
    ...
    def export_markdown(self, output_file: str) -> None:
        ...
        for status in ["critical", "high", "low", "very_low", "none"]:
        ...
        for status in ["critical", "high", "low", "very_low", "none"]:
            ...
            md.append("")
            md.append("| Module | Tests | Sources | Test-File Ratio |")
            md.append("|--------|-------|---------|-----------------|")
