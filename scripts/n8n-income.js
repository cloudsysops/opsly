  ...
  'bitcoin-production': {
    ...
    nodes: [
      { name: 'Invoice Paid', type: 'n8n-nodes-base.webhook', path: 'btc-payment' },
      {
        ...
        url: 'https://api.bitcoin-exchange.com/v1/convert',
      },
      { name: 'Log Income', type: 'n8n-nodes-base.supabase', table: 'platform.btc_income' },
      {
        name: 'Notify Discord',
        type: 'n8n-nodes-base.discord',
        text: '💰 BTC earned: ${{$json.amount_btc}}',
      },
    ],
  },
  'cost-optimization': {
    ...
    nodes: [
      ...
      {
        name: 'Route to Ollama',
        type: 'n8n-nodes-base.httpRequest',
        url: 'http://100.80.41.29:11434/api/generate',
      },
  ...
  'auto-deploy': {
    ...
    nodes: [
      { name: 'GitHub Push', type: 'n8n-nodes-base.webhook', path: 'github-push' },
      {
        ...
        command:
          'cd /opt/opsly && git pull && docker compose -f infra/docker-compose.platform.yml up -d',
      },
...
const strategyArg =
  args.find((a) => a.startsWith('--strategy='))?.split('=')[1] || 'bitcoin-production';
...
console.log(
  `Next: curl -X POST https://n8n-smiletripcare.ops.smiletripcare.com/webhook/import-workflow -d @workflow.json`
);
