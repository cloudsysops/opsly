...
describe.skipIf(skip)('icso-gaming-tbd channel', () => {
  ...
  it('uses gameplay editorial metadata for gaming and preserves the non-gaming explainer default', async () => {
    const baseDir = mkdtempSync(path.join(os.tmpdir(), 'gaming-editorial-'));
    mkdirSync(path.join(baseDir, 'config', 'content-channels'), { recursive: true });
    writeFileSync(
      path.join(baseDir, 'config', 'content-channels', 'icso-gaming-tbd.json'),
      JSON.stringify({
    ...
    const fixture = path.join(baseDir, 'valorant-clutch.mp4');
    await generateOwnedFixture(fixture, 4);

    const gaming = await ingestOwnedVideo({
      tenantId: 'icso-gaming-tbd',
      filePath: fixture,
      title: 'Valorant clutch',
      mode: 'original',
      baseDir,
    });
    expect(gaming.project.formatTemplate).toBe('GAMEPLAY_HIGHLIGHT');
    expect(gaming.project.goal).toBe('engagement');
    expect(gaming.project.portal).toBe('MOVE');
    expect(gaming.project.question).toBe('Highlight: Valorant clutch');

    const nonGaming = await ingestOwnedVideo({
      tenantId: 'intcloudsysops',
      filePath: fixture,
      title: 'Opsly explainer',
      mode: 'original',
      baseDir,
    });
    expect(nonGaming.project.formatTemplate).toBe('NOVA_EXPLAINS');
    expect(nonGaming.project.goal).toBe('education');
    expect(nonGaming.project.portal).toBe('FUTURE');
    expect(nonGaming.project.question).toBe('¿Puede una IA reemplazar a un programador?');
  });
