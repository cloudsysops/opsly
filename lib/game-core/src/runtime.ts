...
import { completeMissionStep, connectNodes, startMission } from './mission.js';
...
export interface GameRuntime {
  ...
  completeMissionStep(sessionId: string, stepId: string): MissionResult;
...
export function createGameRuntime(options?: { now?: () => Date }): GameRuntime {
  ...
  return {
    ...
    completeMissionStep: (sessionId, stepId) => completeMissionStep(store, sessionId, stepId, now),
