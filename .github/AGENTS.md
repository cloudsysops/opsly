...
| Skills operativos | `skills/user/*`, `skills/README.md`; metadata opcional `apps/skill-manifest` (`@intcloudsysops/skill-manifest`) |
...
4. **✅ Skills (manifest opcional)** — *Hecho (2026-04-08).* Paquete `apps/skill-manifest` (`@intcloudsysops/skill-manifest`): `loadSkillPackage`, `parseSkillMarkdown` (frontmatter **JSON** o **YAML simple**), `validateAllUserSkills`, CLI vía `npm run validate-skills`. `manifest.json` opcional con `name`, `version`, `description`, `inputSchema` / `outputSchema`. Pilotos: `skills/user/opsly-api/manifest.json`, `skills/user/opsly-context/manifest.json`. Tests: `apps/skill-manifest/__tests__/manifest.test.ts`. Doc: `skills/README.md`; CI: `.github/workflows/validate-context.yml`.
...
5. **Orchestrator (fases posteriores)** — descomposición / prioridad por plan (§ incremento 6) — siguiente foco documental cuando se priorice.
...
|---|---|---|
| 2026-04-08 | Skills: paquete `apps/skill-manifest` (`@intcloudsysops/skill-manifest`); `manifest.json` + frontmatter JSON o YAML simple; `validateAllUserSkills` exige que `manifest.name` coincida con carpeta bajo `skills/user/` si está definido | Fase 4 incremento 4: metadatos opcionales sin romper `SKILL.md` sin frontmatter |
