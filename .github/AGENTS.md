...
11. **✅ Portal — consumo de uso LLM en dashboard** — *Hecho (2026-04-09).* `apps/portal`: `fetchPortalUsage` + `requirePortalPayloadWithUsage` (`lib/tenant.ts`, `lib/portal-server.ts`); tarjeta **`LlmUsageCard`** en `/dashboard/developer` y `/dashboard/managed` (períodos **hoy** y **mes** en paralelo; fallo de API de uso → mensaje sin tumbar el panel). Tipos: `PortalUsagePayload`, `PortalUsageSnapshot`. Validación: `npm run type-check --workspace=@intcloudsysops/portal`, `npm run lint --workspace=@intcloudsysops/portal`; API **146** tests en verde.
...
11. **Rutas `/api/portal/.../[slug]`** — cuando se priorice: `resolveTrustedPortalSession` + `tenantSlugMatchesSession` (checklist `docs/SECURITY_CHECKLIST.md`).
...
2. **API** — `GET /api/portal/me`, `POST /api/portal/mode`, `GET /api/portal/usage`, invitaciones `POST /api/invitations` + Resend; **`lib/portal-me.ts`**, **`portal-auth.ts`**, **`cors-origins.ts`**, **`apps/api/middleware.ts`**. Portal consume **`/api/portal/usage`** desde servidor con el mismo JWT (`fetchPortalUsage` en `lib/tenant.ts`).
...
- **`GET /api/portal/usage`** — `app/api/portal/usage/route.ts`. Tras `resolveTrustedPortalSession`, **`getTenantUsage(slug, period)`** (`@intcloudsysops/llm-gateway/logger`), alineado con admin **`GET /api/metrics/tenant/:slug`** pero sin segmento `slug` en la URL (solo el tenant de la sesión). Query opcional **`?period=today`** (por defecto) o **`month`**.
...
| 2026-04-09 | Portal dashboards: `requirePortalPayloadWithUsage` + `LlmUsageCard`; mismas credenciales que `/me` para `/api/portal/usage`; degradación si la API de uso falla | Fase 4 incremento 11; sin nuevas rutas dinámicas `[slug]` |
