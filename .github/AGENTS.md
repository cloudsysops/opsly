...
3. **✅ Logs estructurados** — *Hecho (2026-04-05).* Workers: `observability/worker-log.ts` (`worker_start` \| `worker_complete` \| `worker_fail`). LLM Gateway: `structured-log.ts`; `llmCall` registra `llm_call_complete` / `llm_call_error` (opcional `request_id` en `LLMRequest`). Pruebas: `apps/orchestrator/__tests__/worker-log.test.ts`, `apps/llm-gateway/__tests__/structured-log.test.ts`.
...
3. Routing opcional en `llm-gateway` con defaults preservados (siguiente foco plan Fase 4 § incremento 5).
...
| 2026-04-05 | Workers BullMQ + `llmCall`: líneas JSON en stdout (`worker_*`, `llm_call_*`); `LLMRequest.request_id` opcional (UUID si falta) | Misma convención que `job-log.ts`; no sustituye `usage_events` / `logUsage` |
