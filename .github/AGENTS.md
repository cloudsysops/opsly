...
7. **✅ Refuerzo Zero-Trust incremental (feedback)** — *Hecho (2026-04-08).* `POST /api/feedback`: identidad vía `Authorization: Bearer` + `resolveTrustedFeedbackIdentity` (`apps/api/lib/portal-feedback-auth.ts`), alineado a `/api/portal/me` (slug en metadata + `owner_email` en `platform.tenants`); cuerpo no sustituye tenant/email (`parseFeedbackPostFields`); `verifyConversationBelongsToUser` valida `conversation_id`. Portal: `FeedbackChat` con Bearer. Tests integración: `apps/api/__tests__/feedback.test.ts`. **Cierre verificación (misma fecha):** tests unitarios `apps/api/lib/__tests__/portal-feedback-auth.test.ts` (JWT ausente, tenant/metadata, BD, owner); checklist aplicación en `docs/SECURITY_CHECKLIST.md`. Próximo: extender patrón a otras rutas portal sensibles.
...
7. **Ampliar Zero-Trust** a más rutas portal/API sensibles — cuando se priorice (mismo patrón `resolveTrustedFeedbackIdentity` / helpers dedicados).
