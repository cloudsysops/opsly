...
**Fecha última actualización:** 2026-04-07 (tarde) — **Docs + tests + health:** `docs/INVITATIONS_RUNBOOK.md`, `apps/admin/INVITATIONS_UI.md`, `infra/templates/n8n-workflows-template.md`, `config/doppler-audit-2026-04-07.md` (solo nombres de claves). **`GET /api/health`**: `timestamp`, `version` (desde `apps/api/package.json`), `checks.supabase` (probe ligero auth health), `checks.redis` (config presente \| skipped). **Vitest** en **`@intcloudsysops/api`** (`npm run test`) — 6 tests **`invitation-admin-flow`**. Scripts: **`scripts/test-e2e-invite-flow.sh`**, **`scripts/generate-tenant-config.sh`**; **`onboard-tenant.sh`** `--help` + **dry-run** sin exigir env Supabase. **Portal:** tipos en **`apps/portal/types/index.ts`** (`@/types`). Logs invitaciones sin volcar errores crudos. **Pendiente:** onboard `peskids`; UI admin invitations Fase 2.
